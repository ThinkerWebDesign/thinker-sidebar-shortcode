...
class Thinker_Sidebar_Shortcode {
	 ...
	 * If neither ID nor Name exactly matches an active sidebar then returns nothing.
	 *
	 * @since     1.0.0
	 *
	 ...
	 * @return    string    $html     Displays sidebar.
	 */
	public function shortcode( $atts, $content = null ) {
		...
		if ( isset ( $wp_registered_sidebars ) ) {
			...
			if ( is_array ( $atts ) ) {
				...
				// Sets name, id, and class variables from shortcode attributes.
				$name = $atts['name'];
				$id = $atts['id'];
				$class = $atts['class'];
				...
				// Checks if any registered sidebar has active widgets.
				if ( is_dynamic_sidebar() ) {

					// Gives precedent to ID over Name if shortcode ID exactly matches a sidebar.
